...
for (const runtime of ['bun', 'node']) {
  test(`standalone app serves assets and relays a ZIP under ${runtime}`, { timeout: 20_000 }, async () => {
    ...
    try {
      ...
      const integrations = await fetch(`${origin}/integrations?path=/story/editor--playground&globals=framework:vue`, { redirect: 'manual' })
      assert.equal(integrations.status, 302)
      assert.equal(integrations.headers.get('location'), '/integrations/?path=/story/editor--playground&globals=framework:vue')
      const workbench = await fetch(`${origin}/integrations/`)
      assert.equal(workbench.status, 200)
      assert.match(await workbench.text(), /storybook/i)
      const stories = await fetch(`${origin}/integrations/index.json`)
      assert.ok((await stories.json()).entries['editor--playground'])
      const preview = await fetch(`${origin}/integrations/iframe.html`)
      assert.equal(preview.status, 200)
      const previewHtml = await preview.text()
      const module = previewHtml.match(/<script[^>]+type="module"[^>]+src="([^"]+)"/)
      assert.ok(module, 'Storybook preview must load its compiled module')
      const script = await fetch(new URL(module[1]!, `${origin}/integrations/iframe.html`))
