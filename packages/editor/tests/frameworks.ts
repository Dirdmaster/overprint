...
import { EditorRoot, Canvas, Toolbar, Layers, useEditor, useEditorState, useEditorHistory } from '../dist/react.js'
...
const VueCustom = defineComponent({ setup() {
  ...
  const history = VueEditor.useEditorHistory()
  return () => [vh('button', { onClick: () => editor.setTool('paint') }, `Custom ${state.value.tool}`),
    vh('button', { onClick: history.checkpoint, disabled: history.pending.value }, 'Checkpoint'),
    vh('button', { onClick: history.restore, disabled: !history.canRestore.value }, 'Restore checkpoint')]
} })
...
const Custom = () => {
  ...
  return h('nav', null, h('button', { onClick: () => editor.setTool('paint') }, `Custom ${state.tool}`),
    h('button', { onClick: history.checkpoint, disabled: history.pending }, 'Checkpoint'),
    h('button', { onClick: history.restore, disabled: !history.canRestore }, 'Restore checkpoint'))
}
